###############################################################################
# Codes from CMS 94-sample.py
# https://github.com/NSLS-II-CMS/profile_collection/blob/master/startup/94-sample.py
...
class Sample_Generic(CoordinateSystem):
    ...
    def measureRock(self, incident_angle=None, exposure_time=None, extra=None, measure_type='measure', rock_motor=None, rock_motor_limits=0.1, verbosity=3, stitchback=False, poling_period=0.2, **md):
        ...
        max_exposure_time = 0
        for detector in get_beamline().detector:
            if detector.name in set('pilatus300','pilatus800','pilatus2M','pilatus8002'):
...
class SampleStage(Stage):
    ...
    def _set_axes_definitions(self):
        ...
        # The _axes_definitions array holds a list of dicts, each defining an axis
        # at OPLS, we are interested in x2 and sh, # HZ

        self._axes_definitions = [ {'name': 'x2',
                            'motor': x2,
                            'enabled': True,
                            'scaling': +1.0,
                            'units': 'mm',
...
#################################################################################
# Codes from CMS, 95-sample-custom.py
# https://github.com/NSLS-II-CMS/profile_collection/blob/master/startup/95-sample-custom.py
...
class CapillaryHolder(PositionalHolder):
    ...
    def __init__(self, name='CapillaryHolder', base=None, **kwargs):
        ...
        self.x2setOrigin(-17.2) # HZ
        self.shsetOrigin(-1.8)  
...
class TroughChamber(PositionalHolder):
    '''This class is a sample holder that has 3 troughs inside environment chamber.'''

    # Core methods
    ########################################

    def __init__(self, name='TroughChamber', base=None, **kwargs):
        ...
        self.x_spacing = 50 # 2 in 12 mL troughs
        # trough 1, x2 = -64 (front)
        # trough 2, x2 = -14 (middle)
        # trough 3, x2 = 35  (back)
        ...
        # Set the x and y origin to be the center of middle trough
        self.x2setOrigin(-14) # HZ
        self.shsetOrigin(0)  
        
        self.mark('right edge', x2=+45)
        self.mark('left edge', x2=-70)
        self.mark('bottom edge', sh=-30)
        self.mark('center', x2=-14, sh=0)
...
if True:
    ...
    hol = TroughChamber(base=stg)
    hol.addSampleSlot( SampleGISAXS_Generic('test_sample_01'), 1 )
    hol.addSampleSlot( SampleGISAXS_Generic('test_sample_02'), 2 )
    hol.addSampleSlot( SampleGISAXS_Generic('test_sample_03'), 3 )
