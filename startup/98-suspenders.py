...
# Count on XBPM2 suspender
susp_xbpm2_sum = SuspendFloor(xbpm2.sumY, 0.3, resume_thresh=0.8, sleep=300)
RE.install_suspender(susp_xbpm2_sum)
...
# Ring current suspender
susp_beam = SuspendFloor(ring.current, 50, resume_thresh=350, sleep=300, pre_plan = feedback_plan_off, post_plan = feedback_plan_on)
RE.install_suspender(susp_beam)

# Front end shutter suspender
susp_smi_shutter = SuspendFloor(smi_shutter_enable, 0.1, resume_thresh=0.9, sleep=300)
RE.install_suspender(susp_smi_shutter)
