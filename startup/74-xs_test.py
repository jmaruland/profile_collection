...
def reflection_fluorescence_scan_test(scan_param, i, detector='xs', md={}, tilt_stage=False,x2_nominal=0):
    ...
    for alpha in np.linspace(alpha_start, alpha_stop, number_points):
        ...
        yield from bps.mv(exposure_time, exp_time_set)
        xs.settings.acquire_time.set(exp_time_set)
        ...
        yield from bps.mv(shutter, 1)
        xs.settings.num_images.value=number_frames
# Trigger mode must be internal for this to work
